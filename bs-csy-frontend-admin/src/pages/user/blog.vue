<template>
  <div>
    <el-form :inline="true" :model="searchText" class="demo-form-inline">
      <el-form-item label="模糊匹配">
        <el-input v-model="searchText" placeholder="根据标题与文章查询" clearable/>
      </el-form-item>
      <el-form-item>
  ...
  <el-table :data="tableData" stripe style="width: 100%">
    ...
    <el-table-column prop="likedNum" label="点赞数量"/>
...
import {blogByAdmin, disabledTeam, joinUser, teamPageByAdmin} from "~/api/page";
...
const getList = async (currentPage, searchText) => {
  const res = await blogByAdmin(currentPage, searchText);
  console.log(res)
  tableData.value = res.data.records.map(record => ({
    ...record,
    authName: record.author.username
  }));
